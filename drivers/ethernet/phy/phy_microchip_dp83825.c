...
#define DT_DRV_COMPAT microchip_dp83825
...
#define LOG_MODULE_NAME phy_mc_dp83825
#define LOG_LEVEL CONFIG_PHY_LOG_LEVEL
#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(LOG_MODULE_NAME);

#define PHY_MC_DP83825_OMSO_REG			0x0U
#define PHY_MC_DP83825_OMSO_FACTORY_MODE_MASK	0x0U
#define PHY_MC_DP83825_OMSO_NAND_TREE_MASK	0x0U

// #define PHY_MC_DP83825_CTRL2_REG		0x00
#define PHY_MC_DP83825_CTRL2_REG		0x17U
#define PHY_MC_DP83825_CTRL2_REF_CLK_SEL	BIT(7)

#define PHY_MC_DP83825_RESET_HOLD_TIME
...
static int phy_mc_dp83825_read(const struct device *dev,
				uint16_t reg_addr, uint32_t *data)
{
	// LOG_INF("PHY Read");
	const struct mc_dp83825_config *config = dev->config;
	int ret;
...
static int phy_mc_dp83825_write(const struct device *dev,
				uint16_t reg_addr, uint32_t data)
{
	// LOG_INF("PHY Write");
	const struct mc_dp83825_config *config = dev->config;
...
static int phy_mc_dp83825_autonegotiate(const struct device *dev)
{
	// LOG_INF("PHY Autonegotiate");
	const struct mc_dp83825_config *config = dev->config;
	int ret;
	uint32_t bmcr = 0;
	uint32_t bmsr = 0;
	...
	ret = phy_mc_dp83825_read(dev, MII_BMCR, &bmcr);
	if (ret) {
	...
	ret = phy_mc_dp83825_write(dev, MII_BMCR, bmcr);
	if (ret) {
	...
	do {
		...
		ret = phy_mc_dp83825_read(dev, MII_BMSR, &bmsr);
		if (ret) {
...
static int phy_mc_dp83825_get_link(const struct device *dev,
					struct phy_link_state *state)
{
	// LOG_INF("PHY Get Link");
	const struct mc_dp83825_config *config = dev->config;
	struct mc_dp83825_data *data = dev->data;
	int ret;
	uint32_t bmsr = 0;
	uint32_t anar = 0;
	uint32_t anlpar = 0;
	...
	ret = phy_mc_dp83825_read(dev, MII_ANAR, &anar);
	if (ret) {
	...
	ret = phy_mc_dp83825_read(dev, MII_ANLPAR, &anlpar);
	if (ret) {
 ...
 */
static int phy_mc_dp83825_static_cfg(const struct device *dev)
{
	// LOG_INF("PHY Static Configuration");
	const struct mc_dp83825_config *config = dev->config;
	uint32_t omso = 0;
	uint32_t ctrl2 = 0;
	int ret = 0;
	...
	ret = phy_mc_dp83825_read(dev, PHY_MC_DP83825_OMSO_REG, (uint32_t *)&omso);
	if (ret) {
		return ret;
	}

	omso &= ~PHY_MC_DP83825_OMSO_FACTORY_MODE_MASK &
		~PHY_MC_DP83825_OMSO_NAND_TREE_MASK;

	ret = phy_mc_dp83825_write(dev, PHY_MC_DP83825_OMSO_REG, (uint32_t)omso);
	if (ret) {
		return ret;
	}

	/* Select correct reference clock mode depending on interface setup */
	ret = phy_mc_dp83825_read(dev, PHY_MC_DP83825_CTRL2_REG, (uint32_t *)&ctrl2);
	if (ret) {
		return ret;
	}

	if (config->phy_iface == DP83825_RMII) {
		ctrl2 |= PHY_MC_DP83825_CTRL2_REF_CLK_SEL;
	} else {
		ctrl2 &= ~PHY_MC_DP83825_CTRL2_REF_CLK_SEL;
	}

	ret = phy_mc_dp83825_write(dev, PHY_MC_DP83825_CTRL2_REG, (uint32_t)ctrl2);
	if (ret) {
		return ret;
	}
...
static int phy_mc_dp83825_reset(const struct device *dev)
{
	// LOG_INF("PHY Reset");
#if DT_ANY_INST_HAS_PROP_STATUS_OKAY(mc_reset_gpio)
	const struct mc_dp83825_config *config = dev->config;
...
skip_reset_gpio:
#endif /* DT_ANY_INST_HAS_PROP_STATUS_OKAY(mc_reset_gpio) */
	ret = phy_mc_dp83825_write(dev, MII_BMCR, MII_BMCR_RESET);
	if (ret) {
		goto done;
	}
	/* Wait for 500 ms as specified by datasheet */
...
static int phy_mc_dp83825_cfg_link(const struct device *dev,
					enum phy_link_speed speeds)
{
	// LOG_INF("PHY Configure Link");
	const struct mc_dp83825_config *config = dev->config;
	struct mc_dp83825_data *data = dev->data;
	struct phy_link_state state = {};
	int ret;
	...
	/* Reset PHY */
	ret = phy_mc_dp83825_reset(dev);
	if (ret) {
		goto done;
	}

	/* DT configurations */
	ret = phy_mc_dp83825_static_cfg(dev);
	if (ret) {
		goto done;
	}
	...
	ret = phy_mc_dp83825_write(dev, MII_ANAR, anar);
	if (ret) {
	...
	ret = phy_mc_dp83825_autonegotiate(dev);
	if (ret && (ret != -ENETDOWN)) {
	...
	/* Get link status */
	ret = phy_mc_dp83825_get_link(dev, &state);
...
static int phy_mc_dp83825_link_cb_set(const struct device *dev,
					phy_callback_t cb, void *user_data)
{
	// LOG_INF("PHY Link Callback Set");
	struct mc_dp83825_data *data = dev->data;

	data->cb = cb;
	data->cb_data = user_data;

	phy_mc_dp83825_get_link(dev, &data->state);
...
static void phy_mc_dp83825_monitor_work_handler(struct k_work *work)
{
	// LOG_INF("PHY Monitor Work Handler");
	struct k_work_delayable *dwork = k_work_delayable_from_work(work);
	struct mc_dp83825_data *data =
		CONTAINER_OF(dwork, struct mc_dp83825_data, phy_monitor_work);
	const struct device *dev = data->dev;
	struct phy_link_state state = {};
	int rc;

	rc = phy_mc_dp83825_get_link(dev, &state);
...
static int phy_mc_dp83825_init(const struct device *dev)
{
	// LOG_INF("PHY INIT");
	const struct mc_dp83825_config *config = dev->config;
	struct mc_dp83825_data *data = dev->data;
...
skip_int_gpio:
	...
	k_work_init_delayable(&data->phy_monitor_work,
				phy_mc_dp83825_monitor_work_handler);

	return 0;
}

static const struct ethphy_driver_api mc_dp83825_phy_api = {
	.get_link = phy_mc_dp83825_get_link,
	.cfg_link = phy_mc_dp83825_cfg_link,
	.link_cb_set = phy_mc_dp83825_link_cb_set,
	.read = phy_mc_dp83825_read,
	.write = phy_mc_dp83825_write,
};
...
#define MICROCHIP_DP83825_INIT(n)						\
	static const struct mc_dp83825_config mc_dp83825_##n##_config = {	\
		...
		INTERRUPT_GPIO(n)						\
										...
										\
	static struct mc_dp83825_data mc_dp83825_##n##_data;			\
										\
	DEVICE_DT_INST_DEFINE(n, &phy_mc_dp83825_init, NULL,			\
			&mc_dp83825_##n##_data, &mc_dp83825_##n##_config,	\
			POST_KERNEL, CONFIG_PHY_INIT_PRIORITY,			\
			&mc_dp83825_phy_api);

DT_INST_FOREACH_STATUS_OKAY(MICROCHIP_DP83825_INIT)
